...
### Integraciones — decisiones de la etapa 4 (jul 2026)
- **Solo el mes en curso se genera automáticamente al abrirlo**; un mes pasado sin planilla muestra "Sin planilla generada" (no hay planillas fantasma de antes del estreno del módulo). Las de enero–junio 2026 creadas durante las pruebas se eliminaron.
- **Configuración → Planilla (solo Admin)**: settings (EsSalud %, día de pago del mes — null = fin de mes —, porcentajes futuros de gratificación/CTS) **y edición del catálogo AFP** (tasas de fondo/comisión/seguro, activar/desactivar; una AFP inactiva no se ofrece para fichas nuevas pero se conserva en las existentes). Los cambios de tasas rigen para generaciones/refresh futuros (las filas ya generadas conservan su snapshot).
- **Dashboard**: la tarjeta "Últimos gastos registrados" pasa a **"Próximos egresos"** — arriba la planilla del mes como cuenta por pagar real (pendiente, N empleados, vence según día de pago; si el mes no está generado muestra el estimado; si está al día lo dice), debajo se conservan los últimos gastos.
- **Reporte "Planilla anual"** habilitado en Reportes (permiso `reportes`): Excel con hoja resumen por mes (bruto, descuentos, aportes, neto, pagado vs pendiente, EsSalud) y hoja detalle empleado × mes. "Asistencia mensual" (estudiantes) sigue para R4 — la asistencia de personal ya exporta desde su pestaña.

## Decisiones de R4 (académico)

### Decisiones de arranque (jul 2026)
- **Asistencia de estudiantes diaria por sección** (no por sesión de curso como sugería `TeacherScreen.jsx`): una sola toma al día por aula con estados **Presente / Tardanza / Falta / Justificada (P/T/F/J)**, una fila por estudiante+día (mismo molde que la marcación de personal). La marca el tutor o cualquier docente asignado a la sección. A diferencia del personal, **no se derivan faltas automáticas**: un día sin toma queda sin registro y no computa.
- **El docente solo marca el día de hoy** (hora de Lima) y solo sus secciones; puede re-guardar la toma del día. **Días pasados: solo Admin** — puede registrar una toma olvidada completa, pero cambiar un registro existente es corrección individual con justificación ≥ 10 caracteres y auditoría (patrón R3).
- **Notas por competencias configurables en tabla** (Configuración → Evaluación, por defecto 3 por curso), no las tres casillas fijas C1/C2/C3 del prototipo. El "Logro del bimestre" se **sugiere como promedio** (AD=4…C=1) pero **el docente puede ajustarlo**. Escala literal AD/A/B/C en todos los niveles.
- **Cierre por bimestre**: el docente edita notas solo del periodo EN CURSO; bimestres cerrados son solo lectura y los corrige solo Admin con justificación + auditoría.
- **Canal de avisos en R4: wa.me manual** (como los recordatorios de pensiones), siempre al **contacto principal** de la familia. El aviso de falta, las notificaciones de conducta y los comunicados generan enlaces wa.me uno a uno; el envío masivo automático y la "tasa de lectura" del prototipo quedan para Notificaciones (R5).
- **Conducta según el prototipo**: solo incidencias disciplinarias, tipo en texto libre, gravedad **Leve** (solo registro) / **Moderada** (aviso wa.me al apoderado) / **Grave** (aviso + citación presencial con fecha obligatoria). Estados: Registrada → Apoderado notificado → Citación programada → Cerrada. Registran tutor, auxiliar y administración.
- **Asignación docente por curso×sección** (tabla `CourseAssignment`, fuente de verdad de "qué dicta cada docente"; `Course.teacherId` queda como referencia del prototipo). La **grilla semanal de bloques horarios se posterga** (no es prerequisito de asistencia diaria ni de notas). La página "Horarios" pasa a llamarse "Asignación docente".
- **SIAGIE fuera de R4**: el acta del curso·sección·bimestre exporta en Excel simple (patrón exceljs); el formato oficial SIAGIE se hará cuando se tenga la plantilla real del colegio.
- Etapas: **E1** Asistencia (portal docente "Mis clases" + toma diaria + vista admin + asignación docente) → **E2** Notas (Configuración → Evaluación, registro por competencias, libreta) → **E3** Conducta e incidencias → **E4** Calendario académico (feriados que bloquean asistencia y marcación de personal) + Comunicados + reporte "Asistencia mensual".

### Notas — decisiones de la etapa 2 (jul 2026)
- **Quién captura**: cada docente registra solo los cursos×secciones de su **asignación docente**; el **tutor** del aula registra además los aspectos formativos y la evaluación del apoderado; Admin puede editar todo.
- **Competencias por curso sembradas con el CNEB real por área** (Matemática 4, Comunicación 3, Personal Social 5, etc.), editables en **Configuración → Evaluación** (renombrar/añadir; eliminar solo si aún no tienen notas). Los aspectos de la libreta (formativos + evaluación del apoderado) también son catálogo editable: no se eliminan, se desactivan.
- **Escala AD/A/B/C en todos los niveles, incluido Inicial** (decisión del administrador: la norma no restringe el AD en Inicial).
- **"Logro del bimestre" = SIEMPRE automático** (promedio de competencias, AD=4…C=1, redondeo half-up); **no admite ajuste manual, ni para docente ni para Admin** (corrección post-validación de E2 — reemplaza la idea inicial de logro ajustable). Se muestra como badge de solo lectura. "Condición": promedio ≥2.5 Logrado · ≥1.5 En proceso · resto En inicio.
- **Cierre por bimestre en la práctica**: EN CURSO editable; CERRADO solo Admin con motivo ≥10 (queda en auditoría con old→new, sin columnas de corrección); PRÓXIMO nadie captura. Vaciar una celda mientras el bimestre está en curso borra la nota (captura en curso, no anulación de documento).
- **La libreta solo se imprime/exporta a PDF en R4** — el botón "Enviar al apoderado" del prototipo no se implementa (llega con Notificaciones R5). El pie muestra la asistencia del bimestre (% = (P+T)/total y tardanzas) desde el módulo de E1.
...
### Calendario y comunicados — decisiones de la etapa 4 (jul 2026)
- **Calendario académico** con eventos Feriado / Exámenes / Actividad por año. Los **feriados nacionales del Perú vienen precargados por seed** (editables/eliminables) y Admin añade los días no lectivos propios. Un feriado **bloquea la toma de asistencia de estudiantes** (422 a nivel API, no solo UI) y **neutraliza la falta derivada del personal** ese día (no cuenta como día hábil del resumen ni del export de R3). Las marcas reales y las tardanzas no se recalculan.
- **Los vencimientos de pensión no se registran a mano**: el calendario los deriva del cronograma (fecha efectiva de cuotas pendientes/vencidas, respetando compromisos) y enlazan con Pensiones. Esto reemplaza el tipo "Vencimiento pensiones" manual del prototipo.
- **Comunicados**: **un alcance por comunicado** (todo el colegio, un nivel, un grado o una sección — selector encadenado; para combinaciones se duplica el comunicado). Estados **Borrador → Enviado** (sin "Programado" ni tasa de lectura: llegan con el envío automático de R5). El **envío es manual por wa.me**, familia por familia (contacto principal, deduplicado por apoderado: una familia con dos hijos en el alcance recibe un solo mensaje), y al terminar se marca Enviado con el conteo de familias. Un enviado es histórico: no se edita ni elimina; los borradores sí. Correlativo `C-####`. Una Actividad del calendario puede generar su comunicado borrador con un checkbox.
- **Reporte "Asistencia mensual"** (Reportes, permiso `reportes`): **solo estudiantes** — Excel del mes con hoja Resumen por sección (P/T/F/J y % asistencia) y hoja Detalle por estudiante. La asistencia del personal ya exporta desde su pestaña (R3) y no se duplica.
- Con esto **R4 queda completo**: pendientes que siguen en backlog — export SIAGIE (cuando haya plantilla oficial), grilla de horarios por bloques (con catálogo de horas académicas y recreos, pedida por el administrador), envío digital de libretas y comunicados (Notificaciones R5), promoción de estudiantes → pre-matrículas 2027.

## Decisiones de la grilla de horarios (jul 2026, post-R4 — rumbo a v1.0.0)

Rumbo fijado por el administrador: **v1.0.0 = todo lo construido + grilla de horarios + portal del apoderado**. Pagos en línea, notificaciones automáticas, export SIAGIE e inventario quedan para después de la 1.0.0.

- **Bloques horarios por nivel y turno**, configurables en tabla (hora inicio/fin por bloque, cuáles son recreo con etiqueta), con seed típico de 45 min (mañana y tarde en Primaria/Secundaria; Inicial con su propia plantilla). No se puede eliminar ni volver recreo un bloque que ya tiene clases programadas.
- **La celda del horario asigna solo el curso**: el docente se deriva SIEMPRE de la Asignación docente (curso×sección) — una sola fuente de verdad; si el curso no tiene docente, la celda lo avisa sin bloquear.
- **Choques de docente bloqueados con error** (409 con mensaje de dónde está ocupado), evaluados por **solape de horas** — funciona incluso entre secciones de niveles con plantillas de bloques distintas. Las horas programadas vs las semanales del curso solo se advierten (chips), nunca bloquean.
- **Copiar de otra sección** (mismo nivel y turno): reemplaza el horario destino; los bloques que chocan o cuyos cursos no existen en el grado destino se omiten y se reportan.
- **Vistas**: Admin edita en Horarios (pestañas Horario por sección / Asignación docente / Bloques horarios) e imprime por aula; el **docente ve "Mi horario"** (solo lectura) en su portal. El apoderado lo verá en su portal (siguiente etapa).

## Decisiones del portal del apoderado (jul 2026 — cierre de la v1.0.0)

- **Cuentas generadas por secretaría**: usuario = **DNI del apoderado**, clave temporal aleatoria mostrada UNA sola vez, **cambio de clave obligatorio al primer ingreso** (flujo genérico para cualquier rol). Generación individual desde la ficha del apoderado (con regeneración de clave) y **masiva** ("Generar accesos pendientes") con lista de credenciales imprimible. El correo del usuario se genera sintético (`dni@apoderado.local`): las familias no tienen correo institucional y el login es por DNI.
- **Acceden todos los apoderados vinculados activos** (máx. 3 por estudiante); cada uno ve a TODOS sus hijos con matrícula vigente del año activo (selector de hijo persistente). El contacto principal sigue siendo el único destinatario de los avisos wa.me. Un apoderado sin hijos vigentes puede entrar pero ve el portal vacío.
- **Portal 100% de solo lectura**, mobile-first (sin prototipo en el kit: nace del design system). Módulos: **Inicio** (resumen del hijo: deuda vencida/próxima cuota, asistencia del mes, conducta abierta, próximos eventos, últimos comunicados), **Pagos** (cronograma con estados/mora e historial con nº de recibo — sin descarga de PDF), **Asistencia** (mes P/T/F/J + horario semanal de su sección + calendario), **Notas** (SOLO bimestres CERRADOS, cursos y aspectos; la libreta oficial imprimible sigue siendo exclusiva de administración) y **Avisos** (comunicados del alcance familiar + incidencias de conducta del hijo, sin las anuladas ni datos internos).
