...
Las **decisiones de negocio de R2** (mora fija una vez, sin pagos parciales, caja compartida por día con cobro solo con caja abierta, recordatorios wa.me, devolución sin saldo a favor, catálogo de conceptos de venta sin stock) están en `alcance-funcional.md` § "Decisiones de arranque R2". Plan por etapas: **E1** Caja y cobros (sesión, cobro multi-cuota + conceptos, recibo, anulación, arqueo) → **E2** Pensiones + job de mora + recordatorios → **E3** Compromisos + Devoluciones (flujos de dos pasos) → **E4** Tesorería (gastos, otros ingresos, caja chica, resumen) → **E5** Dashboard económico + exportaciones.
