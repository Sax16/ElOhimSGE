...
### Cierre técnico de R4 — etapa 3 (conducta e incidencias)

- **Módulo `conduct`** (apps/api/src/conduct/): `ConductIncident` con correlativo `I-####` (CodeCounter 'conduct'), enums `ConductSeverity` (LEVE/MODERADA/GRAVE) y `ConductStatus` (REGISTRADA/APODERADO_NOTIFICADO/CITACION_PROGRAMADA/CERRADA/ANULADA) espejados en shared con labels/tones. Permiso del módulo: **`notas`** (mapeo de NAV_MODULE; no existe permiso `conducta` propio).
- **State machine en service**: `POST /:id/notified` (tras abrir wa.me) avanza REGISTRADA→APODERADO_NOTIFICADO (moderada) o →CITACION_PROGRAMADA (grave); reenviar solo refresca `notifiedAt`; LEVE→422; estados finales→409. `close` y `cancel` (solo ADMIN, motivo ≥10) con AuditLog `conduct.*`. Año CERRADO→409. La citación es obligatoria en GRAVE vía superRefine del schema zod.
- **Scope por rol** idéntico a student-attendance: docente = tutorías ∪ asignaciones del año activo, aplicado a lista, stats, students, detalle y mutaciones. Stats (mes, graves, abiertas, citaciones de la semana L–D) calculadas en fronteras de día de Lima y sobre el scope del usuario.
- **Front** (apps/web/src/features/conduct/): pantalla única `/conducta` para admin y docente (el scope lo pone el API); registro con alerts condicionales por gravedad, detalle con flujo wa.me manual (`conductWaUrl` + confirmación a `/notified`), cierre y anulación (patrón ReasonDialog). Filas anuladas atenuadas; tabla → tarjetas en sm.
- Seed `13-r4-conducta.ts`: 5 incidencias demo idempotentes (por code) cubriendo los cinco estados no anulados.

### Cierre técnico de R4 — etapa 4 (calendario, comunicados, reporte de asistencia)

- **Módulo `calendar`** (permiso `estructura`): `CalendarEvent` (FERIADO/EXAMEN/ACTIVIDAD, rangos startDate..endDate @db.Date, año activo). `common/holidays.util.ts` expone `getHolidaySet(prisma, from, to)` — ÚNICA fuente de feriados para las integraciones. Vencimientos de pensión derivados en el GET por fecha efectiva de cuotas PENDIENTE/VENCIDO (installment-view, respeta compromisos).
- **Retrofit feriados**: student-attendance (PUT 422 + roster editable=false en feriado) y staff attendance R3 (un día es hábil solo si L–V y no feriado: la falta derivada y el export mensual lo excluyen; tardanzas y planilla intactas).
- **Módulo `announcements`** (permiso `comunicados`): `Announcement` C-#### (CodeCounter 'announcement'), alcance COLEGIO/NIVEL/GRADO/SECCION con FKs opcionales validadas por superRefine, BORRADOR editable/eliminable vs ENVIADO inmutable (409). `GET /:id/recipients` deduplica por apoderado principal (una familia = una entrada con todos sus hijos del alcance); `POST /:id/send` congela sentAt/recipientsCount. El front hace el envío manual wa.me por familia y luego marca enviado.
- **Reporte `student-attendance`** en reports (permiso `reportes`): preview por sección + xlsx con hojas Resumen y Detalle; % = (P+T)/total. La tarjeta de Reportes quedó habilitada (ReportKey `studentAttendance`).
- **Front**: `/calendario` (grilla mensual desktop / agenda por día en sm según responsive.md; NAV_MODULE.calendario pasó de null a `estructura`) y `/comunicados` (composición con alcance encadenado + dialog de envío con checklist local por familia). Chips de pensión navegan a `/pagos` (la ruta real del módulo Pensiones — el prototipo decía "pagos" con otro id).
...
## 11. Cierre técnico — portal del apoderado (v1.0.0)

- **Acceso**: `Guardian.userId String? @unique` → User; `User.mustChangePassword` se activa con cada clave temporal. `POST /guardians/:id/access` crea/regenera (username = DNI, clave legible de 8 sin ambiguos, correo sintético `dni@apoderado.local` por la unicidad de email); `POST /guardians/access/bulk` genera los pendientes (apoderados activos con hijos vigentes) y devuelve las credenciales una sola vez. `POST /auth/change-password` (genérico) limpia el flag; el front lo intercepta en RequireAuth antes de cualquier ruta.
- **Módulo `portal`** (apps/api/src/portal/, prefijo /portal): sin `@RequirePermission` (el rol APODERADO no tiene módulos) — el service valida rol APODERADO + Guardian vinculado y la **propiedad de cada enrollmentId** (hijo vigente del año activo; ajeno → 404, otros roles → 403). Reutiliza la lógica existente: cuotas por fecha efectiva y `installment.status` (montos string decimal, recibo/paidAt derivados del Receipt EMITIDO), notas vía `GradesService.portalGrades` (filtro CERRADO), horario/calendario/conducta/comunicados de sus módulos.
- **Front** (apps/web/src/features/portal/): GUARDIAN_NAV (`phome/ppagos/pasist/pnotas/pavisos`), `HOME_BY_ROLE.APODERADO='/phome'`, selector de hijo persistente (Zustand), mobile-first (una columna, targets ≥44px, grid 2-col desde 560px), botón wa.me a secretaría con el teléfono institucional. Gestión de acceso en la ficha de Apoderados + dialog masivo con lista imprimible (`printCredentials.ts`).
