...
export class EnrollmentService {
  ...
  // GET /api/dashboard/summary — resumen del año activo (cualquier rol autenticado).
  async dashboardSummary() {
    ...
    const [activeStudents, total, byType, todayCount, weekCount, levels] = await Promise.all([
      this.prisma.student.count({ where: { status: { in: ['ACTIVO', 'BECADO'] } } }),
      this.prisma.enrollment.count({ where: activeWhere }),
      this.prisma.enrollment.groupBy({ by: ['type'], where: activeWhere, _count: { _all: true } }),
      this.prisma.enrollment.count({ where: { ...activeWhere, enrolledAt: { gte: startOfToday } } }),
      this.prisma.enrollment.count({ where: { ...activeWhere, enrolledAt: { gte: startOfWeek } } }),
      this.prisma.level.findMany({
        ...
        select: {
          id: true,
          name: true,
          grades: { select: { sections: { select: { id: true, capacity: true } } } },
        },
      }),
    ]);
    ...
    // Matrículas activas por nivel (una query agregada por sección).
    const enrolledByLevel = new Map<string, number>();
    const perSection = await this.prisma.enrollment.groupBy({
      by: ['sectionId'],
      where: activeWhere,
      _count: { _all: true },
    });
    const sectionCount = new Map(perSection.map((s) => [s.sectionId, s._count._all]));

    let totalCapacity = 0;
    const byLevel = levels.map((l) => {
      let capacity = 0;
      let enrolled = 0;
      for (const grade of l.grades) {
        for (const section of grade.sections) {
          capacity += section.capacity;
          enrolled += sectionCount.get(section.id) ?? 0;
        }
      }
      enrolledByLevel.set(l.id, enrolled);
      totalCapacity += capacity;
      return { levelName: l.name, enrolled, capacity };
    });

    return {
      yearName: year.name,
      students: { active: activeStudents },
      enrollments: { total, byType: typeCounts, today: todayCount, week: weekCount },
      vacancies: { capacity: totalCapacity, enrolled: total, free: totalCapacity - total },
      byLevel,
    };
  }
}
