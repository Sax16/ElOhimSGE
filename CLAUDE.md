...
**R1 (núcleo administrativo) y R2 (dinero) están completos, validados y en el repositorio.** R1: identity (login por usuario o correo, usuarios/roles/permisos), estructura académica (año cerrado = 409 a nivel API), estudiantes y apoderados (N:M, máx. 3, contacto principal único), matrícula con asistente, cronograma de pagos (reglas puras en `packages/shared/src/money/` — 71 tests), tarifario/descuentos. R2: caja y cobros (sesión única por día, recibos `R-YYYY-#####`, arqueo completo con historial), pensiones (mora fija materializada por job pg-boss 00:30, exoneración solo Admin, recordatorios wa.me), compromisos de pago (reprogramación 1:1 con fecha efectiva) y devoluciones (dos pasos), tesorería (gastos/ingresos con categorías en tabla, caja chica con rendición), dashboard económico y reportes con export `.xlsx` (exceljs). Siguiente: **R3 (personal: marcación, planilla)**.

**IMPORTANTE antes de implementar R3+**: las decisiones de negocio que reemplazan partes del diseño original están en **`design/docs/alcance-funcional.md`** (§ "Decisiones de implementación — cierre R1" y §§ "Decisiones de R2" por etapa: mora solo a pensiones escolares, cobro siempre en caja, deuda del estudiante y no de la matrícula viva, etc.) y las precisiones técnicas en **`design/docs/stack-tecnico.md` § 6 (cierre R1) y § 7 (cierre R2 + punteo de arranque de R3)**. Claves de R2 a respetar: fecha efectiva de cuotas centralizada en `apps/api/src/common/installment-view.util.ts`; toda consulta de dinero confía en `installment.status` (nunca en matrícula anulada); la planilla de R3 debe crear su gasto en tesorería por evento (badge de origen). Esas secciones MANDAN sobre los prototipos `.jsx` donde difieran.
